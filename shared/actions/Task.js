...
export function fetchItem(uuid) {
  return function(dispatch) {
    fetch(`${process.env.SERVER_URL}/api/${uuid}`)
      .then(r1 => r1.json())
      .then((r1) => {
        ...
        if (r1.errors) {
          ...
          fetch(`${process.env.SERVER_URL}/api/${uuid}/transactions`)
            .then(r2 => r2.json())
            .then((r2) => {
              
              fetch(`${process.env.SERVER_URL}/api/${uuid}/events`)
                .then(r3 => r3.json())
                .then((r3) => {
...
export function fetchAccount(uuid) {
  return function(dispatch) {
    fetch(`${process.env.SERVER_URL}/api/transactions?account_id=${uuid}`)
      .then(r1 => r1.json())
      .then((r1) => {
        ...
        if (r1.errors) {
          ...
          fetch(`${process.env.SERVER_URL}/api/batch_transactions?account_id=${uuid}`)
            .then(r2 => r2.json())
            .then((r2) => {
